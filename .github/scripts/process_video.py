...
def process_video(src, dst, climber, route, grade):
           """
               Draw a semi-transparent black banner at the TOP of the video
                   with the text:  {climber} | {route} {grade}
                       """
    ...
    # filter_complex:
    # 1) drawbox – black banner across full width, top of frame
    # 2) drawtext – centered white text inside the banner
    vf = (
...
def upload_file(svc, path, folder_id, name):
           """Upload video using OAuth user token (resumable upload) if available,
...
def update_sheet(creds, status, output_file_id=None, error=None):
    ...
    ws.update_cell(SHEET_ROW, 7, status)           # G = "Done" / "Error"
    if output_file_id:
                   ws.update_cell(SHEET_ROW, 8, output_file_id)  # H = output file id
    if error:
                   ws.update_cell(SHEET_ROW, 9, str(error)[:500])  # I = error msg
