...
export function useCreateProgram() {
  ...
  return useMutation({
    mutationFn: (body: { title: string; description?: string; isPrivate?: boolean; tags?: string[] }) =>
...
export function useUpdateProgram() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...body }: { id: string; title?: string; description?: string; isPrivate?: boolean; tags?: string[] }) =>
...
export function useAssignProgram() {
  return useMutation({
    mutationFn: ({ id, ...body }: { id: string; clientId: string; startDate: string }) =>
...
export function useAddProgramWeek() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ programId, ...body }: { programId: string; title?: string; notes?: string }) =>
